 ...
 * @param color 颜色
 */
void OLED_DrawImageInverted(uint8_t x, uint8_t y, const Image *img, OLED_ColorMode color) {
  uint8_t fullRow = img->h / 8; // 完整的行数
  uint8_t partBit = img->h % 8; // 不完整的字节中的有效位数
  uint16_t dataIndex;
  ...
  for (uint8_t i = 0; i < img->w; i++) {
    for (uint8_t j = 0; j < fullRow; j++) {
      // 计算原始数据索引（从下往上读取）
      dataIndex = i + (fullRow - 1 - j) * img->w;
      // 读取原始数据并反转位（因为OLED数据格式是高位在下）
      invertedData = 0;
      for (uint8_t bit = 0; bit < 8; bit++) {
        invertedData |= ((img->data[dataIndex] >> bit) & 0x01) << (7 - bit);
      }
      // 写入显存
      OLED_SetBits(x + i, y + j * 8, invertedData, color);
    }
  }
  
  if (partBit) {
    uint16_t fullNum = img->w * fullRow; // 完整的字节数
    ...
    for (uint8_t i = 0; i < img->w; i++) {
      ...
      // 读取原始数据并反转位
      invertedData = 0;
      for (uint8_t bit = 0; bit < partBit; bit++) {
        invertedData |= ((img->data[dataIndex] >> bit) & 0x01) << (7 - bit);
      }
      // 处理不完整行的情况
      OLED_SetBits_Fine(x + i, y + (fullRow * 8), invertedData, partBit, color);
    }
  }
}
